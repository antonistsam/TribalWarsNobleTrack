...
(async function () {
    ...
    function fillUnitAmountsFakeLimitWorld() {
        ...
        if (scouts > 0 && fakePop > 0) {
            let scoutsToSend = scouts;
            if (fakePop < 2) {
                scoutsToSend = 1;
            } else {
                if (scouts > maxScouts) {
                    scoutsToSend = maxScouts;
                }
                // Don't exceed available population
                let maxScoutsForPop = Math.floor(fakePop / 2);
                scoutsToSend = Math.min(scoutsToSend, maxScoutsForPop);
            }
            if (scoutsToSend > 0) {
                document.forms[0].spy.value = scoutsToSend;
                fakePop = fakePop - scoutsToSend * 2;
            }
        }

        if (fakePop > 0) {
            // Prioritize Spear (1 pop each)
            if (spear > 0 && fakePop > 0) {
                let spearToSend = Math.min(spear, fakePop);
                document.forms[0].spear.value = spearToSend;
                fakePop = fakePop - spearToSend;
            }
            // Then Axe (1 pop each)
            if (axe > 0 && fakePop > 0) {
                let axeToSend = Math.min(axe, fakePop);
